"use client"

import { useState } from "react"
import { useRouter } from "next/navigation"
import Link from "next/link"
import { useAuth, type UserRole } from "@/contexts/auth-context"
import { Bus, ArrowRight, Loader2, User, Truck, Check } from "lucide-react"
import { MobileInput } from "@/components/auth/mobile-input"
import { OwnerBusinessFields } from "@/components/auth/owner-business-fields"

export default function SignupPage() {
  const [email, setEmail] = useState("")
  const [password, setPassword] = useState("")
  const [name, setName] = useState("")
  const [mobile, setMobile] = useState("")
  const [role, setRole] = useState<UserRole>("customer")
  
  // Bus Owner specific fields
  const [fleetName, setFleetName] = useState("")
  const [operatingCity, setOperatingCity] = useState("")
  const [licenseDocument, setLicenseDocument] = useState<File | null>(null)
  
  const [error, setError] = useState("")
  const { signup, isLoading } = useAuth()
  const router = useRouter()

  const validateMobile = (value: string) => {
    const digits = value.replace(/\D/g, "")
    return digits.length >= 10
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError("")
    
    // Basic validation
    if (!email || !password || !name || !mobile) {
      setError("Please fill in all required fields")
      return
    }

    if (!validateMobile(mobile)) {
      setError("Please enter a valid 10-digit mobile number")
      return
    }

    // Bus Owner specific validation
    if (role === "owner") {
      if (!fleetName || !operatingCity || !licenseDocument) {
        setError("Please complete all business information fields")
        return
      }
    }

    try {
      await signup({
        email,
        password,
        name,
        mobile,
        role,
        ...(role === "owner" && {
          fleetName,
          operatingCity,
          licenseDocument: licenseDocument?.name,
        }),
      })
      router.push("/")
    } catch {
      setError("Something went wrong")
    }
  }

  return (
    <div className="min-h-screen flex">
      {/* Left Panel - Branding */}
      <div className="hidden lg:flex lg:w-1/2 bg-primary items-center justify-center p-12">
        <div className="max-w-md">
          <div className="flex items-center gap-3 mb-8">
            <Bus className="h-10 w-10 text-primary-foreground" strokeWidth={1.5} />
            <span className="text-2xl font-light tracking-tight text-primary-foreground">BusConnect</span>
          </div>
          <h1 className="text-4xl font-light text-primary-foreground leading-tight mb-4 text-balance">
            Join the future of bus travel
          </h1>
          <p className="text-primary-foreground/70 text-lg font-light">
            Whether you are a traveler or fleet owner, we have got you covered.
          </p>
        </div>
      </div>

      {/* Right Panel - Form */}
      <div className="flex-1 flex items-center justify-center p-8 lg:p-12 overflow-y-auto">
        <div className="w-full max-w-sm">
          {/* Mobile Logo */}
          <div className="flex items-center gap-2 mb-12 lg:hidden">
            <Bus className="h-8 w-8 text-foreground" strokeWidth={1.5} />
            <span className="text-xl font-light tracking-tight">BusConnect</span>
          </div>

          <div className="mb-8">
            <h2 className="text-2xl font-light tracking-tight mb-2">Create account</h2>
            <p className="text-muted-foreground text-sm">Start your journey with BusConnect</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Role Selection */}
            <div className="space-y-3">
              <label className="text-xs uppercase tracking-wider text-muted-foreground">I am a</label>
              <div className="grid grid-cols-2 gap-3">
                <button
                  type="button"
                  onClick={() => setRole("customer")}
                  className={`relative flex flex-col items-center gap-2 p-4 border transition-all ${
                    role === "customer"
                      ? "border-foreground bg-muted/50"
                      : "border-border hover:border-muted-foreground/50"
                  }`}
                >
                  {role === "customer" && (
                    <div className="absolute top-2 right-2">
                      <Check className="h-3 w-3 text-foreground" />
                    </div>
                  )}
                  <User className="h-5 w-5 text-muted-foreground" strokeWidth={1.5} />
                  <span className="text-sm">Customer</span>
                </button>
                <button
                  type="button"
                  onClick={() => setRole("owner")}
                  className={`relative flex flex-col items-center gap-2 p-4 border transition-all ${
                    role === "owner"
                      ? "border-foreground bg-muted/50"
                      : "border-border hover:border-muted-foreground/50"
                  }`}
                >
                  {role === "owner" && (
                    <div className="absolute top-2 right-2">
                      <Check className="h-3 w-3 text-foreground" />
                    </div>
                  )}
                  <Truck className="h-5 w-5 text-muted-foreground" strokeWidth={1.5} />
                  <span className="text-sm">Bus Owner</span>
                </button>
              </div>
              {role === "owner" && (
                <p className="text-xs text-muted-foreground">
                  Bus owner accounts require approval. You will get 30 days free trial upon approval.
                </p>
              )}
            </div>

            {/* Common Fields */}
            <div className="space-y-4">
              <div className="relative">
                <input
                  type="text"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="Full name"
                  className="w-full px-0 py-3 bg-transparent border-0 border-b border-border text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-foreground transition-colors text-sm"
                  autoComplete="name"
                />
              </div>
              <div className="relative">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="Email address"
                  className="w-full px-0 py-3 bg-transparent border-0 border-b border-border text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-foreground transition-colors text-sm"
                  autoComplete="email"
                />
              </div>
              <MobileInput 
                value={mobile} 
                onChange={setMobile} 
                placeholder="Mobile number"
              />
              <div className="relative">
                <input
                  type="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="Password"
                  className="w-full px-0 py-3 bg-transparent border-0 border-b border-border text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-foreground transition-colors text-sm"
                  autoComplete="new-password"
                />
              </div>
            </div>

            {/* Bus Owner Business Fields */}
            {role === "owner" && (
              <OwnerBusinessFields
                fleetName={fleetName}
                operatingCity={operatingCity}
                licenseDocument={licenseDocument}
                onFleetNameChange={setFleetName}
                onOperatingCityChange={setOperatingCity}
                onLicenseDocumentChange={setLicenseDocument}
              />
            )}

            {error && (
              <p className="text-destructive text-xs">{error}</p>
            )}

            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 bg-primary text-primary-foreground py-3 px-4 text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
                <>
                  Create account
                  <ArrowRight className="h-4 w-4" />
                </>
              )}
            </button>
          </form>

          <div className="mt-8 pt-8 border-t border-border">
            <p className="text-center text-sm text-muted-foreground">
              Already have an account?{" "}
              <Link href="/login" className="text-foreground hover:underline underline-offset-4">
                Sign in
              </Link>
            </p>
          </div>
        </div>
      </div>
    </div>
  )
}
